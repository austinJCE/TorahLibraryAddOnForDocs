...
function searchVoices(query, options) {
  ...
  return hits.map(function(hit, index) {
    ...
    return {
      ...
      // Frontend hook stubs — the insert UI reads and sends these back on insert.
      insertMode: 'reference',   // 'reference' | 'contents' | 'both'
      includeReference: true,
      includeContents: false,

      // Hidden/defaulted controls not exposed in the primary insert UI.
      showMediaLabel: true,
 ...
 * Keeps current behavior unless the frontend passes a different mode.
 */
...
function searchLexicon(query) {
  ...
  // Route English input to reverse-lookup path
...
function searchLexiconByEnglish_(query) {
  ...
  // Extract unique Hebrew headwords from refs like "Klein Dictionary, תּוֹרָה 1"
  var seenHw = {};
  var headwords = [];
  ...
  // Limit re-fetches; fetch full definitions for each unique headword
  var results = [];
  var idx = 0;
 ...
 *
 * frontendOptions.insertMode: 'reference' | 'contents' | 'both'
 * sheetPayload fields forwarded from the sidebar result object:
 *   insertMode, includeReference, includeContents,
 *   showMediaLabel, preserveSheetSpacing, debugUnknownNodes
 */
...
/**
 * Normalize insert options from both the payload and explicit frontendOptions.
 * Priority: frontendOptions.insertMode > sheetPayload.insertMode > 'reference'.
 * Boolean overrides on sheetPayload.includeReference / includeContents are respected.
 */
 ...
 * Preserves paragraph spacing and converts common entities.
 */
