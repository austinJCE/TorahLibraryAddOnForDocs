...
| --- | ---------- | ------- | ------------------- | ------------ | ---- |
| Divine-name substitution silently stopped applying on insert for upgrading users. | rewrite branch introduction of `apply_sheimot_on_insertion` (~2026-02) | — | **new opt-in gate without migration**: a new preference key was added defaulting to `false`; existing users who had substitutions on before never got them turned back on. | `test/tests/hebrew-preferences.test.js` — "applies Hebrew divine-name replacements after display normalization" (asserts that with the gate set, substitution happens). The Stage 2 migration test will pin the upgrade path. | Never add a new preference key that gates existing behavior without a default of `true` *and* an `apply_sheimot_on_insertion`-style migration. |
| `extendedGemaraPreference` module-scope global set but only read on sidebar-open paths; Quick-Actions menu callers see stale `false`. | rewrite branch refactor of `openSharedSidebar_` | — | **dead-scoped global**: the setter is conditional, the reader runs unconditionally. | Stage 2 will add a `test/tests/extended-gemara.test.js` that invokes `insertRichTextFromHTML` with the preference both `"true"` and `"false"` and asserts on the stripped/kept `<strong>` and `<i>` markup. | Module-scope globals that hold preferences are a smell. Read preferences at call time, not at sidebar-open time. |
| `formatDataForPesukim` appends a newline even when `pesukim` is `false`, changing inserted-document spacing. | rewrite branch (`Code.gs:759`) | — | **output-shape change without a snapshot test**. | Stage 2 will add a `test/tests/insertion-output.test.js` snapshot of `formatDataForPesukim` output on a two-verse fixture for both `pesukim=true` and `pesukim=false`. | Insertion-path output has snapshot tests. Do not "simplify" formatting paths without first updating the snapshot. |
| "Refresh Sidebar" button in preferences calls `google.script.run.refreshSidebarAfterPreferences()` — server function does not exist, click silently no-ops. | unknown (the client call is at `apps-script/preferences/js.html:791`, the server binding was either renamed or never landed) | — | **server refactor renames/removes a function the client still calls**: the failure path is silent, because no `withFailureHandler` is attached. | `test/ui/rpc-surface.test.js` — "every google.script.run.X call in client HTML resolves to a surface-listed function". The broken call is currently listed in `docs/rpc-surface.json` under `_knownBrokenCalls`; Stage 2 is scheduled to restore the binding and delete the entry. | Every client `google.script.run.X` call must resolve to a server function listed in `docs/rpc-surface.json`. The contract test enforces it. |
