...
function buildReferenceInsertPlan(data, options, context) {
  if (options.singleLanguage) {
    ...
    const blocks = [
      ...
      },
      {
        type: 'body',
        lang: options.singleLanguage,
        html: isHebrew ? data.he : data.text
      }
    ];

    if (isHebrew && context.transliterationText) {
      blocks.push({
        type: 'transliteration',
        text: context.transliterationText,
        ltr: true
      });
    }
  ...
  if (context.resolvedBilingualLayout == 'he-top') {
    ...
    if (context.transliterationText) {
      blocks.push({
        type: 'transliteration',
        text: context.transliterationText,
        ltr: true
  ...
  return {
    ...
    hebrewText: data.he,
    transliterationText: context.transliterationText,
...
function insertReferenceTransliterationParagraph(doc, index, block, context) {
  return insertTransliterationParagraphAfter(doc, index, block.text, context.typography, block.ltr !== false);
}
  ...
  plan.blocks.forEach(function(block) {
    switch (block.type) {
      ...
      case 'transliteration':
        insertReferenceTransliterationParagraph(doc, currentIndex, block, context);
        currentIndex += 1;
        break;
      case 'attribution': {
...
function insertReferenceCellBody(cell, lang, html, context) {
  let paragraph = cell.setText('').insertParagraph(0, '');
  paragraph.setLeftToRight(lang != 'he');
  paragraph.setAttributes(context.styles.nullStyle);
...
function renderReferenceTablePlan(doc, index, plan, data, context) {
  ...
  insertReferenceCellBody(table.getCell(1, plan.hebrewColumn), 'he', plan.hebrewText, context);

  if (plan.transliterationText) {
    insertTransliterationIntoCell(table.getCell(1, plan.hebrewColumn), plan.transliterationText, context.typography);
  }
