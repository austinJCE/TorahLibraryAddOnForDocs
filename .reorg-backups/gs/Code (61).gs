...
const SIDEBAR_SESSION_CACHE_PREFIX = 'sidebar_session:';
const ACTIVE_SIDEBAR_SESSION_ID_KEY = 'active_sidebar_session_id';
const SIDEBAR_SESSION_TTL_SECONDS = 21600;
...
function onInstall() {
  ...
  setAccountPreferences(basicPrefs);
...
function onOpen(e) {
  ...
  if (!e || e.authMode !== ScriptApp.AuthMode.NONE) {
    ...
    if (prefs.popcorn_enabled == "true") {
      addOnMenu.addSeparator().addItem('Popcorn (beta, legacy feature by Shlomi Helfgot)', 'popcornHTML');
    }
  }
...
function getSidebarBootstrapData(mode, sessionId) {
  ...
  const resolvedSessionId = generateSidebarSessionId_(sessionId);
  const sessionState = resolvedMode === 'advanced' ? getSidebarSessionState(resolvedSessionId) : {};
  const effectivePreferences = mergeEffectivePreferences_(accountPreferences, sessionState);
  PropertiesService.getUserProperties().setProperty(ACTIVE_SIDEBAR_SESSION_ID_KEY, resolvedSessionId);
  return {
    mode: resolvedMode,
...
function insertReference(data, singleLanguage = undefined, pasukPreference = true, preferredTitle = null, includeTranslationSourceInfo = false, bilingualLayout = "he-right", insertSefariaLink = false, includeTransliteration = false, insertCitationOnly = false) {
  ...
  */
  ...
  if (insertCitationOnly === true || insertCitationOnly === "true") {
    let citationText = (singleLanguage == "he") ? (data.heRef || title) : title;
    let citationParagraph = doc.insertParagraph(index, citationText)
      .setAttributes(headerStyle)
      .setLeftToRight(singleLanguage == "he" ? false : true);
...
function readStoredPreferences_() {
  ...
  for (let i = 0; i < SETTINGS.length; i++) {
    ...
    returnPrefs[key] = (storedValue !== null && storedValue !== undefined) ? storedValue : defaults[key];
  }
  return returnPrefs;
}

function writeStoredPreferences_(preferenceObject) {
  const userProperties = PropertiesService.getUserProperties();
  for (const property in (preferenceObject || {})) {
    if (!Object.prototype.hasOwnProperty.call(preferenceObject, property)) continue;
    try {
  ...
  for (let i = 1; i < arguments.length; i++) {
    ...
    Object.keys(source).forEach(function(key) {
      ...
      if (Array.isArray(value)) {
        ...
        const base = output[key] && Object.prototype.toString.call(output[key]) === '[object Object]' ? output[key] : {};
        output[key] = mergeDeep_(base, value);
      } else {
        output[key] = value;
      }
    });
...
function mapSessionStateToAccountPreferences_(sessionState) {
  const mapped = {};
  ...
  if (Object.prototype.hasOwnProperty.call(source, 'include_transliteration')) mapped.include_transliteration_default = source.include_transliteration;
  if (Object.prototype.hasOwnProperty.call(source, 'insert_citation')) mapped.insert_citation_default = source.insert_citation;
  if (Object.prototype.hasOwnProperty.call(source, 'insert_sefaria_link')) mapped.insert_sefaria_link_default = source.insert_sefaria_link;
  if (Object.prototype.hasOwnProperty.call(source, 'show_line_markers')) mapped.show_line_markers_default = source.show_line_markers;
  if (Object.prototype.hasOwnProperty.call(source, 'output_mode')) mapped.output_mode_default = source.output_mode;
  if (Object.prototype.hasOwnProperty.call(source, 'bilingual_layout')) mapped.bilingual_layout_default = source.bilingual_layout;

  return mapped;
}

function mergeEffectivePreferences_(accountPreferences, sessionState) {
  const effective = mergeDeep_({}, accountPreferences || {});
  const source = sessionState || {};
  if (Object.prototype.hasOwnProperty.call(source, 'include_translation_source_info')) effective.include_translation_source_info = source.include_translation_source_info;
  if (Object.prototype.hasOwnProperty.call(source, 'nekudot')) effective.nekudot = source.nekudot;
  if (Object.prototype.hasOwnProperty.call(source, 'teamim')) effective.teamim = source.teamim;
  if (Object.prototype.hasOwnProperty.call(source, 'include_transliteration')) effective.include_transliteration_default = source.include_transliteration;
  if (Object.prototype.hasOwnProperty.call(source, 'insert_citation')) effective.insert_citation_default = source.insert_citation;
  if (Object.prototype.hasOwnProperty.call(source, 'insert_sefaria_link')) effective.insert_sefaria_link_default = source.insert_sefaria_link;
  if (Object.prototype.hasOwnProperty.call(source, 'show_line_markers')) effective.show_line_markers_default = source.show_line_markers;
  if (Object.prototype.hasOwnProperty.call(source, 'output_mode')) effective.output_mode_default = source.output_mode;
  if (Object.prototype.hasOwnProperty.call(source, 'bilingual_layout')) effective.bilingual_layout_default = source.bilingual_layout;
  if (Object.prototype.hasOwnProperty.call(source, 'last_translation_languages')) effective.last_translation_languages = source.last_translation_languages;
  if (Object.prototype.hasOwnProperty.call(source, 'last_translation_only_filter')) effective.last_translation_only_filter = source.last_translation_only_filter;
  if (Object.prototype.hasOwnProperty.call(source, 'last_search_sort_mode')) effective.last_search_sort_mode = source.last_search_sort_mode;
  if (Object.prototype.hasOwnProperty.call(source, 'last_search_relevance_sort')) effective.last_search_relevance_sort = source.last_search_relevance_sort;
  return effective;
}

function generateSidebarSessionId_(sessionId) {
...
function getSidebarSessionCacheKey_(sessionId) {
  return SIDEBAR_SESSION_CACHE_PREFIX + generateSidebarSessionId_(sessionId);
}

function readSessionState_(sessionId) {
  ...
  try {
    ...
    Logger.log(`Could not parse sidebar session state: ${error.message}`);
    return {};
  }
}

function writeSessionState_(sessionId, state) {
  CacheService.getUserCache().put(getSidebarSessionCacheKey_(sessionId), JSON.stringify(state || {}), SIDEBAR_SESSION_TTL_SECONDS);
}

function deleteSessionState_(sessionId) {
  CacheService.getUserCache().remove(getSidebarSessionCacheKey_(sessionId));
}

function getAccountPreferences() {
  return readStoredPreferences_();
}

function setAccountPreferences(preferenceObject) {
  const current = getAccountPreferences();
  const merged = mergeDeep_({}, current, preferenceObject || {});
  writeStoredPreferences_(merged);
  return merged;
}

function getSidebarSessionState(sessionId) {
  return readSessionState_(sessionId);
}

function setSidebarSessionState(sessionId, statePatch) {
  const resolvedSessionId = generateSidebarSessionId_(sessionId);
  const current = getSidebarSessionState(resolvedSessionId);
  const merged = mergeDeep_({}, current, statePatch || {});
  writeSessionState_(resolvedSessionId, merged);
  PropertiesService.getUserProperties().setProperty(ACTIVE_SIDEBAR_SESSION_ID_KEY, resolvedSessionId);
  return merged;
}
...
function clearSidebarSessionState(sessionId) {
  const resolvedSessionId = generateSidebarSessionId_(sessionId);
  deleteSessionState_(resolvedSessionId);
  return true;
}

function saveSidebarSessionAsAccountDefaults(sessionId) {
  const sessionState = getSidebarSessionState(sessionId);
  const mappedDefaults = mapSessionStateToAccountPreferences_(sessionState);
  return setAccountPreferences(mappedDefaults);
}

function refreshSidebarAfterPreferences() {
  const activeSessionId = PropertiesService.getUserProperties().getProperty(ACTIVE_SIDEBAR_SESSION_ID_KEY);
  if (activeSessionId) {
    clearSidebarSessionState(activeSessionId);
  }
  return true;
}
...
function setPreferences(preferenceObject) {
  return setAccountPreferences(preferenceObject);
}
...
function runQuickActionLearningCycleByType(kind) {
  assertExperimentalAiEnabled_();
...
function runQuickActionTodaysDafLessonMenu(){ return runQuickActionLearningCycleMenu_('daf', "today's Daf lesson"); }
function runQuickActionTodays929LessonMenu(){ return runQuickActionLearningCycleMenu_('929', "today's 929 lesson"); }
function runQuickActionThisWeeksParashahLessonMenu(){ return runQuickActionLearningCycleMenu_('parashah', "this week's Parashah lesson"); }
function runQuickActionLearningCycleMenu_(kind, label) {
  try {
