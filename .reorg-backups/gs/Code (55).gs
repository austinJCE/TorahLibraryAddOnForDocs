...
function onOpen(e) {
  ...
  if (!e || e.authMode !== ScriptApp.AuthMode.NONE) {
    ...
    if (prefs.experimental_ai_source_sheet_enabled == "true") {
      addOnMenu
        .addSeparator()
        .addItem('Generate Shiur Draft (experimental)', 'openAiLessonGeneratorStub');
      quickActionsMenu.addSeparator()
        .addItem("Today's Learning Cycle Shiur (45 min)", 'openLearningCycleShiurStub');
    }
...
function openAiLessonGeneratorStub() {
  const output = HtmlService.createHtmlOutputFromFile('ai_lesson_stub')
    .setWidth(420)
    .setHeight(520);
  DocumentApp.getUi().showModalDialog(output, 'AI Lesson Generator (experimental)');
}

function openLearningCycleShiurStub() {
  const output = HtmlService.createHtmlOutputFromFile('ai_lesson_stub')
    .setWidth(420)
    .setHeight(520);
  DocumentApp.getUi().showModalDialog(output, "Today's Learning Cycle Shiur (experimental)");
}
