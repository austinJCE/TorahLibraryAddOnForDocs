...
function howItWorksPopup() {
  let html = HtmlService.createHtmlOutputFromFile('help')
      .setWidth(820)
      .setHeight(860);
  DocumentApp.getUi().showModalDialog(html, 'Using the sidebar');
}
...
function applyHeaderStyleWithTypography(paragraph, isHebrew, typography, useLinkedStyle) {
  const prefs = getPreferences();
  const font = useLinkedStyle ? (prefs.sefaria_link_font || (isHebrew ? typography.hebrewFont : typography.translationFont))
    : (prefs.source_title_font || (isHebrew ? typography.hebrewFont : typography.translationFont));
  const size = Number(useLinkedStyle ? (prefs.sefaria_link_font_size || (isHebrew ? typography.hebrewFontSize : typography.translationFontSize))
    : (prefs.source_title_font_size || (isHebrew ? typography.hebrewFontSize : typography.translationFontSize)));
  const style = useLinkedStyle ? (prefs.sefaria_link_font_style || 'bold')
    : (prefs.source_title_font_style || 'bold');
  applyTypographyToParagraph(paragraph, font, size, style);
}
...
function maybeBuildTransliterationText_(data, includeTransliteration, currentPrefs) {
  ...
  const transliterationIsBiblical = String(currentPrefs.transliteration_is_biblical_hebrew) !== "false";
  const isBiblicalHebrewText = transliterationIsBiblical && data.type == "Tanakh";
  let transliterationOverrides = {};
  try {
    transliterationOverrides = JSON.parse(currentPrefs.transliteration_overrides || '{}') || {};
  } catch (error) {}
  return transliterateHebrewHtmlPreservingBasicBreaks(data.he, transliterationScheme, {
    keepNiqqud: true,
...
function insertReference(data, singleLanguage = undefined, pasukPreference = true, preferredTitle = null, includeTranslationSourceInfo = false, bilingualLayout = "he-right", insertSefariaLink = false, includeTransliteration = false) {
  ...
  if (cursor) {
    ...
    while (currentElement && currentElement.getParent() && currentElement.getParent() !== doc) {
      currentElement = currentElement.getParent();
    }
    if (currentElement && currentElement.getParent() === doc) {
      index = doc.getChildIndex(currentElement) + 1;
    }
  }
  /* ---- test harness --- 
  ...
  */
  ...
  if (singleLanguage) {
    ...
    if (resolvedBilingualLayout == "he-top") {
      ...
      if (transliterationText) {
        appendTransliterationToParagraph(hebTextParagraph, transliterationText, typography);
      }
      ...
      applyTypographyToParagraph(engTitle, typography.translationFont, typography.translationFontSize, typography.translationFontStyle);
      if (insertSefariaLink) {
      ...
      applyTypographyToParagraph(hebTitle, typography.hebrewFont, typography.hebrewFontSize, typography.hebrewFontStyle);
      if (insertSefariaLink) {
...
function getPreferences() {
  ...
  const returnPrefs = Object.assign({}, defaults);
  const preferences = PropertiesService.getUserProperties();
  const keys = (typeof SETTINGS !== 'undefined' && Array.isArray(SETTINGS)) ? SETTINGS.slice() : [];
  Object.keys(defaults).forEach(function(key) {
    if (keys.indexOf(key) < 0) keys.push(key);
  });
  keys.forEach(function(key) {
    const value = preferences.getProperty(key);
    if (value !== null && value !== undefined) {
      returnPrefs[key] = value;
    }
  });
  return returnPrefs;
}
