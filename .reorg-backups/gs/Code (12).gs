...
function insertReference(data, singleLanguage = undefined, pasukPreference = true, preferredTitle = null, includeTranslationSourceInfo = false, bilingualLayout = "he-right", insertSefariaLink = false, includeTransliteration = false) {
  ...
  */
  ...
  const isBiblicalHebrew = String(currentPrefs.transliteration_is_biblical_hebrew || "false") === "true" || String((data && data.type) || "").toLowerCase() === "tanakh";
  const biblicalDageshMode = currentPrefs.transliteration_biblical_dagesh_mode || "none";
  const transliterationText = (includeTransliteration && data.he)
    ? transliterateHebrewHtmlPreservingBasicBreaks(data.he, transliterationScheme, {
        keepNiqqud: true,
        isBiblicalHebrew: isBiblicalHebrew,
        dageshMode: biblicalDageshMode
      })
    : "";
