...
function openSharedSidebar_(mode) {
  ...
  // Search-rescue rollback: use the last known working search sidebar for text search
  // until the unified sidebar search pipeline is stabilized.
  let template = HtmlService.createTemplateFromFile('main');
  let output = template.evaluate()
    .setTitle(resolvedMode === 'advanced' ? 'Advanced Search' : 'Find & Insert Source')
    .setWidth(300);
  DocumentApp.getUi().showSidebar(output);
  extendedGemaraPreference = PropertiesService.getUserProperties().getProperty('extended_gemara');
}
...
function getSidebarBootstrapData(mode) {
  const prefs = getPreferences();
