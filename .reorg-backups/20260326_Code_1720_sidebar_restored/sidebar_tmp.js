...
// Consolidated/organized pass: merged late overrides into base
// ============================================================
...
function updateVersionControlsVisibility() {
  var outputMode = $('.output-mode-selection').val();
  var translationCount = $('.translation-version-select option:not(:disabled)').length;
  var hebrewCount = $('.he-version-selection option:not(:disabled)').length;
  var hideTranslations = translationCount <= 1;
  var hideOriginals = hebrewCount <= 1;

  $('.translation-version-details').toggle(outputMode !== 'he' && appMode !== 'voices' && !hideTranslations);
  $('.translation-version-control').toggle(outputMode !== 'he' && appMode !== 'voices' && !hideTranslations);
  $('.advanced-version-details').toggle(outputMode !== 'en' && appMode === 'advanced' && !hideOriginals);

  $('.translation-version-count-inline').text(hideTranslations ? '' : '(' + translationCount + ' available)');
  $('.original-version-count-inline').text(hideOriginals ? '' : '(' + hebrewCount + ' available)');
  $('.versions-section').toggleClass('single-translation', hideTranslations);

  if (appMode === 'voices' || !selectedResultKey) {
    $('.versions-section').hide();
    return;
  }

  $('.versions-section').toggle(!hideTranslations || (!hideOriginals && appMode === 'advanced' && outputMode !== 'en'));
}
...
function syncCompactTranslationLanguageUI() {
  ...
  if (!anySelected && selected.length) {
    label = selected.slice(0, 2).map(function (lang) {
      return labelMap[lang] || String(lang || '').toUpperCase();
    }).join(', ');
...
function updateTranslationLanguageVisibility() {
  ...
  var anyChecked = $('.translation-language-any').is(':checked');
  var useAny = anyChecked || !selectedValues.length;
...
function renderResultGroup(selector, items, groupName) {
  ...
  items.forEach(function (item) {
    ...
    var titleText = groupName === 'Search results' && item.ref ? item.ref : (item.label || item.ref);
...
function updateSuggestion(dataIn, inputTitle) {
  ...
  $('.translation-version-count-inline').text(translationEntries.length > 1 ? '(' + translationEntries.length + ' available)' : '');
  $('.original-version-count-inline').text(hebrewEntries.length > 1 ? '(' + hebrewEntries.length + ' available)' : '');
  $('.translation-version-details').toggle(translationEntries.length > 1);
  $('.advanced-version-details').toggle(hebrewEntries.length > 1);
...
function setSidebarMode(mode, skipPreferenceSave) {
  ...
  $('main.unified-sidebar').attr('data-mode', appMode);
...
function updateModeDependentSections() {
  ...
  updateCompactSelectorSummaries();
  syncSessionOverrideButtons();
...
function updateCompactSelectorSummaries() {
  ...
  $('#display-choice-summary-text').text(modeLabelMap[mode] || 'Bilingual');
  $('#layout-choice-summary-text').text(layoutLabelMap[layout] || 'Right–Left');
  $('.layout-selector-shell').toggle(mode === 'both');

  $('.output-mode-button').each(function () {
    var isSelected = $(this).attr('data-value') === mode;
    $(this).toggleClass('selected', isSelected).attr('aria-checked', isSelected ? 'true' : 'false');
  });

  $('.bilingual-layout-button').each(function () {
    var isSelected = $(this).attr('data-value') === layout;
    $(this).toggleClass('selected', isSelected).attr('aria-checked', isSelected ? 'true' : 'false');
  });

  var $sourceIcon = $('.bilingual-layout-button.selected .layout-icon').first().clone();
  if ($sourceIcon.length) {
    $('#layout-selector-icon').empty().append($sourceIcon);
  }
}

function closeCompactSelectorPanels() {
  $('#display-selector-panel, #layout-selector-panel').addClass('hidden');
  $('#display-selector-button, #layout-selector-button').attr('aria-expanded', 'false').removeClass('is-open');
}

$('#display-selector-button').off('click').on('click', function (event) {
  event.preventDefault();
  event.stopPropagation();
  var isOpen = !$('#display-selector-panel').hasClass('hidden');
  closeCompactSelectorPanels();
  if (!isOpen) {
    $('#display-selector-panel').removeClass('hidden');
    $('#display-selector-button').attr('aria-expanded', 'true').addClass('is-open');
  }
});

$('#layout-selector-button').off('click').on('click', function (event) {
  event.preventDefault();
  event.stopPropagation();
  var isOpen = !$('#layout-selector-panel').hasClass('hidden');
  closeCompactSelectorPanels();
  if (!isOpen) {
    $('#layout-selector-panel').removeClass('hidden');
    $('#layout-selector-button').attr('aria-expanded', 'true').addClass('is-open');
  }
});

$(document).off('click.compactSelectors').on('click.compactSelectors', function (event) {
  if ($(event.target).closest('.compact-selector-shell').length) return;
  closeCompactSelectorPanels();
});

$('.output-mode-button').off('click').on('click', function (event) {
  event.preventDefault();
  var value = $(this).attr('data-value');
  if (!value) return;

  $('.output-mode-selection').val(value).trigger('change');
  saveSidebarPreference({ output_mode_default: value }, function () {
    updateBilingualLayoutVisibility();
    updateTranslationDetailsVisibility();
    updateHebrewOptionsVisibility();
    updateTransliterationVisibility();
...
$('.bilingual-layout-button').off('click').on('click', function (event) {
  event.preventDefault();
  var value = $(this).attr('data-value');
  if (!value) return;

  $('.bilingual-layout-selection').val(value).trigger('change');
  saveSidebarPreference({ bilingual_layout_default: value }, function () {
    updateCompactSelectorSummaries();
    closeCompactSelectorPanels();
    if (hasResolvedReference && data && data.ref) {
      updateSuggestion(data, selectedResultSummary ? selectedResultSummary.label : data.ref);
...
$('.output-mode-selection').off('change.compactSummary').on('change.compactSummary', function () {
  updateCompactSelectorSummaries();
  updateBilingualLayoutVisibility();
  updateTranslationDetailsVisibility();
  updateHebrewOptionsVisibility();
...
$('.bilingual-layout-selection').off('change.compactSummary').on('change.compactSummary', function () {
  updateCompactSelectorSummaries();
  if (hasResolvedReference && data && data.ref) {
    $('.suggestions').html(renderPreviewHTML(data, selectedResultSummary ? selectedResultSummary.label : data.ref));
  }
});
