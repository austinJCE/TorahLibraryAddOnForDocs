...
function onOpen() {
  const menu = DocumentApp.getUi().createAddonMenu()
      ...
      .addItem('Using the sidebar', 'howItWorksPopup')
...
function maybeBuildTransliterationText_(data, includeTransliteration, currentPrefs) {
  const wantsTransliteration = includeTransliteration === true || String(includeTransliteration) === 'true';
  if (!wantsTransliteration || !data || !data.he) {
    return '';
  }
...
function getPreferences() {
  ...
  for (let i = 0; i < SETTINGS.length; i++) {
    ...
    returnPrefs[key] = value !== null ? value : defaults[key];
  }
  return returnPrefs;
}
