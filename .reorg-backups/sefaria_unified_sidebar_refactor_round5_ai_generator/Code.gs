  ...
  if (!e || e.authMode !== ScriptApp.AuthMode.NONE) {
    ...
    if (prefs.experimental_ai_source_sheet_enabled == "true") {
      ...
      quickActionsMenu.addSeparator()
        .addItem("Today's Learning Cycle Shiur (45 min)", 'runQuickActionTodaysLearningCycleShiurMenu');
    }
...
function runQuickActionTodaysLearningCycleShiur() {
  assertExperimentalAiEnabled_();
  ...
  const payload = {
    ...
    audience: 'Adult learners',
    duration: 45,
    lessonStyle: 'Integrated weekly shiur',
    additionalInstructions: 'Create a teachable 45-minute shiur that meaningfully weaves together today’s learning cycle. Keep the flow practical, coherent, and suitable for live teaching. Review before sharing.',
    includeOriginal: true,
    includeTranslation: true,
    includeEducatorNotes: true,
    includeDiscussionPrompts: true,
    contextMode: 'learning_cycle',
    learningCycleContext: context
  };

  return generateAiLessonDraft(payload);
}

function runQuickActionTodaysLearningCycleShiurMenu() {
  try {
    const result = runQuickActionTodaysLearningCycleShiur();
    DocumentApp.getUi().alert('Generated and inserted: ' + result.title);
    return result;
  } catch (error) {
    DocumentApp.getUi().alert(error && error.message ? error.message : "Unable to generate today's learning cycle shiur.");
    throw error;
  }
}
