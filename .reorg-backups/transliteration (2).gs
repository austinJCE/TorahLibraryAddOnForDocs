/* Hebrew transliteration engine with consolidated scheme support and Biblical Hebrew-aware dagesh handling. */
...
function collectCluster_(text, startIndex) {
  ...
  return {
    ...
    hasAnyVowel: marks.some(function(mark) { return VOWEL_RE.test(mark); }),
    hasFullVowel: marks.some(function(mark) { return VOWEL_RE.test(mark) && mark !== SHEVA; })
  };
}
...
function transliterateCluster_(cluster, scheme, dageshType, options) {
  ...
  for (var i = 0; i < cluster.marks.length; i += 1) {
    ...
    if (Object.prototype.hasOwnProperty.call(scheme.vowels, mark)) vowels += scheme.vowels[mark];
  }
  return cons + vowels;
}
...
function normalizeBiblicalOptions_(options) {
  ...
  if (typeof options.isBiblicalHebrew === "undefined") {
    options.isBiblicalHebrew = !!options.isTanakhText;
  }
  if (typeof options.isTanakhText === "undefined") {
    options.isTanakhText = !!options.isBiblicalHebrew;
  }

  if (!options.dageshMode) options.dageshMode = "none"; // none | forte | all
  return options;
}
...
function transliterateHebrewText(text, schemeKey, options) {
  ...
  for (var i = 0; i < cleaned.length;) {
    ...
    out += transliterateCluster_(cluster, scheme, dageshType, options);
    i = cluster.nextIndex;
  }
