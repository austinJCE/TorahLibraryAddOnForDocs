...
function onOpen(e) {
  ...
  // Use createMenu() so nested submenus render reliably during development.
  // In published editor add-ons, the caption is ignored and the menu is moved
  // under Extensions automatically.
  const menu = ui.createMenu('Sefaria')
  ...
  if (!e || e.authMode !== ScriptApp.AuthMode.NONE) {
    ...
    if (prefs.popcorn_enabled == "true") {
      menu.addSeparator().addItem('Popcorn (beta, legacy feature by Shlomi Helfgot)', 'popcornHTML');
    }
  }
