...
function renderSessionLibrary() {
  ...
  var $filter = $('.session-library-corpus-filter');
  if (!$panel.length) return;
  $pinned.empty(); $inserted.empty();

  var allItems = (sessionLibrary.pinned || []).concat(sessionLibrary.inserted || []);
  var corpora = [];
  var seen = {};
  ...
  var filterHTML = ["<option value=''>All corpora</option>"].concat(corpora.map(function(corpus){
    return "<option value='" + escapeHTML(corpus) + "'" + (currentFilter === corpus ? " selected" : "") + ">" + escapeHTML(corpus) + "</option>";
  }));
  $filter.html(filterHTML.join('')).toggle(corpora.length > 1);

  function renderGrouped($target, items, allowRemove) {
    var groups = {};
    (items || []).forEach(function(item){
      var corpus = String((item && item.corpusLabel) || 'Other');
      if (currentFilter && corpus !== currentFilter) return;
...
function getVisibleCorpusEntries() {
  return resultPostProcessState.availableCorpora.filter(function(entry){ return entry && entry.key; });
}
...
function renderResultFilterStrip() {
  ...
  var currentValue = resultPostProcessState.removedCorpora.length === 1 ? resultPostProcessState.removedCorpora[0] : '';
  var options = ["<option value=''>All corpora</option>"];
  getVisibleCorpusEntries().forEach(function(entry) {
    options.push("<option value='" + escapeHTML(entry.key) + "'" + (currentValue === entry.key ? " selected" : "") + ">Hide " + escapeHTML(entry.label) + "</option>");
  });
  var html = "<div class='result-filter-strip-label'>Corpus filters</div>" +
    "<div class='result-filter-dropdown-shell'><select class='compact-corpus-filter results-corpus-filter' aria-label='Hide one corpus from results'>" + options.join('') + "</select></div>";
...
function resolveSelectedResult(selectedRef, inputLabel) {
  ...
  google.script.run.withSuccessHandler(function (response) {
    ...
    if (nodeState.isInsertable) {
      ...
      $('.display-layout-wrapper').show();
      $('.preview-accordion').prop('open', true);
...
$('.results-panel').on('change', '.results-corpus-filter', function () {
  var corpusKey = $(this).val();
  resultPostProcessState.removedCorpora = corpusKey ? [corpusKey] : [];
  applyResultPostProcessing();
});
...
$('.open-preferences-tool').on('click', function () { google.script.run.preferencesPopup(); });
