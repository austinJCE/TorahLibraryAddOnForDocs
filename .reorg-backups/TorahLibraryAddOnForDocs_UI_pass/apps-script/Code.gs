...
function searchHitHasTranslation_(hit) {
  let source = hit && hit._source ? hit._source : {};
  if (source && Array.isArray(source.availableLangs)) {
    return source.availableLangs.indexOf('en') >= 0;
  }
  if (source && Array.isArray(source.versions)) {
    return source.versions.some(function(version) {
      return version && version.language === 'en';
    });
  }
...
function findSearch(input, filters, pageRank) {
   ...
   let searchConfig = (filters && typeof filters === 'object' && !Array.isArray(filters))
     ? filters
     : { filters: filters, relevanceSort: pageRank };
   let searchOptions = buildSearchWrapperPayload_(input, searchConfig);
  ...
  try {
    ...
    if (searchConfig.translationOnly) {
      hits = hits.filter(searchHitHasTranslation_);
    }
    return hits;
  } catch (error) {
