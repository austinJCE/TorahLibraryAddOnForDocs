...
## Round 7 cleanup
- Removed legacy `basic_tmp.js` from the bundle.
- Removed jQuery UI autocomplete from the unified sidebar; debounced unified search now handles query suggestions/results.
- Fixed malformed `sidebar_js.html` closing script tag so the summary-label updater runs inside the script.
